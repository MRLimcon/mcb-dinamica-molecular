...
mutable struct Config
    ...
    neighbour_boxes::Array{Vector{Int}}
    area::Float32
end
...
function make_config(
    ...
    neighbour_boxes = make_boxes(L, Float32(l_box))
    return Config(
        L,
        T,
        l_box,
...
function calc_lennard_jones_force!(
    ...
    nearby_positions::Matrix{Float32},
    force::Matrix{Float32},
    A::Float32,
    B::Float32,
)
    ...
    for i = 1:n_inside
        if n_nearby > 0
            ...
            r2 =
                r[1:n_nearby, 1] .* r[1:n_nearby, 1] .+
                r[1:n_nearby, 2] .* r[1:n_nearby, 2] .+ r[1:n_nearby, 3] .* r[1:n_nearby, 3]
            norms = sqrt.(r2)
            r4 = r2 .* r2
            r6 = r4 .* r2
            r7 = r6 .* norms
        ...
        if i >= 2
            ...
            r2 =
                r[1:i-1, 1] .* r[1:i-1, 1] .+ r[1:i-1, 2] .* r[1:i-1, 2] .+
                r[1:i-1, 3] .* r[1:i-1, 3]
            norms = sqrt.(r2)
            r4 = r2 .* r2
            r6 = r4 .* r2
...
function calc_force!(particles::Particles, config::Config)
    ...
    for j = 1:size(config.boxes, 1),
        k = 1:size(config.boxes, 2),
        l = 1:size(config.boxes, 3)
    ...
    for j = 1:size(config.boxes, 1),
        ...
        config.neighbour_boxes[j, k, l] = Vector{Int}()
    end
    ...
    @sync Threads.@threads for i = 1:size(particles.positions, 1)
        ...
        i_s = setdiff(max(1, x - 1):min(shape_boxes[1], x + 1), x)
        j_s = setdiff(max(1, y - 1):min(shape_boxes[2], y + 1), y)
        k_s = setdiff(max(1, z - 1):min(shape_boxes[3], z + 1), z)

        push!.(config.neighbour_boxes[i_s, j_s, k_s], i)
    end

    @sync Threads.@threads for i = 1:shape_boxes[1]
        for j = 1:shape_boxes[2], k = 1:shape_boxes[3]
            inside_box_check = config.boxes[i, j, k]
            inside_box_check =
                inside_box_check[1 .<= inside_box_check .<= size(particles.positions, 1)]

            nearby_box_check = config.neighbour_boxes[i, j, k]
            nearby_box_check =
                nearby_box_check[1 .<= nearby_box_check .<= size(particles.positions, 1)]

            if (size(inside_box_check, 1) > 0) ||
               (size(inside_box_check, 1) > 1 && size(nearby_box_check, 1) > 0)
